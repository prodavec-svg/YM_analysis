import argparse
from pathlib import Path
import logging
import duckdb
import shutil

logging.basicConfig(
    filename='build_marts.log',
    level=logging.INFO,
    format='%(asctime)s - %(levelname)s - %(message)s'
)
# Also log to stdout
console = logging.StreamHandler()
console.setLevel(logging.INFO)
logging.getLogger('').addHandler(console)

PROJECT_DIR = Path(__file__).resolve().parents[1]
DEFAULT_INPUT_GLOB = PROJECT_DIR / "data" / "processed" / "multi_event_clean" / "**" / "*.parquet"
DEFAULT_MARTS_DIR = PROJECT_DIR / "data" / "marts"
TMP_DIR = PROJECT_DIR / "data" / "processed" / "tmp_duckdb_marts"

def build_all(input_glob: str, marts_dir: Path) -> None:
    logging.info(f"Starting unified mart compilation using DuckDB. Input: {input_glob}")
    marts_dir.mkdir(parents=True, exist_ok=True)
    TMP_DIR.mkdir(parents=True, exist_ok=True)
    
    con = duckdb.connect(database=':memory:')
    con.execute("PRAGMA memory_limit='8GB'")
    con.execute(f"PRAGMA temp_directory='{TMP_DIR.as_posix()}'")
    
    # 1. Base view
    con.execute(f"""
        CREATE VIEW source AS 
        SELECT *, 
               LEAST(GREATEST(played_ratio_pct, 0.0), 100.0) AS played_ratio_capped_pct,
               (is_listen = true 
                AND sequence_eligible = true 
                AND is_bot_session = false 
                AND is_suspected_bot_user = false) AS eligible
        FROM read_parquet('{input_glob}');
    """)

    logging.info("Compiling mart_unified.parquet (Daily Isolated Logic)...")
    
    query = f"""
    COPY (
        WITH 
        -- 1. Daily User Segments
        user_daily_likes AS (
            SELECT 
                time_period, uid,
                SUM(CASE WHEN event_type = 'like' THEN 1 ELSE 0 END) AS total_likes,
                SUM(CASE WHEN event_type = 'like' AND is_organic = 1 THEN 1 ELSE 0 END) AS organic_likes,
                SUM(CASE WHEN event_type = 'like' AND is_organic = 0 THEN 1 ELSE 0 END) AS algo_likes
            FROM source
            GROUP BY time_period, uid
        ),
        user_segments AS (
            SELECT 
                time_period, uid,
                CASE 
                    WHEN total_likes < 5 THEN 'Cold'
                    WHEN CAST(organic_likes AS FLOAT) / GREATEST(total_likes, 1) > 0.7 THEN 'Explorer'
                    WHEN CAST(algo_likes AS FLOAT) / GREATEST(total_likes, 1) > 0.7 THEN 'Passive'
                    ELSE 'Mixed'
                END AS segment
            FROM user_daily_likes
        ),
        
        -- 2. Daily Content Tiers
        item_daily_likes AS (
            SELECT 
                time_period, item_id,
                SUM(CASE WHEN event_type = 'like' THEN 1 ELSE 0 END) AS total_likes
            FROM source
            GROUP BY time_period, item_id
        ),
        item_ranked AS (
            SELECT 
                time_period, item_id, total_likes,
                PERCENT_RANK() OVER (PARTITION BY time_period ORDER BY total_likes DESC) AS percentile
            FROM item_daily_likes
        ),
        item_tiers AS (
            SELECT 
                time_period, item_id,
                CASE 
                    WHEN percentile <= 0.01 THEN 'Head'
                    WHEN percentile <= 0.20 THEN 'Torso'
                    ELSE 'Tail'
                END AS content_tier
            FROM item_ranked
        ),
        
        -- 3. Join Segments and Tiers back to source events
        enriched_source AS (
            SELECT 
                s.*,
                COALESCE(u.segment, 'Cold') AS segment,
                COALESCE(i.content_tier, 'Tail') AS content_tier
            FROM source s
            LEFT JOIN user_segments u ON s.time_period = u.time_period AND s.uid = u.uid
            LEFT JOIN item_tiers i ON s.time_period = i.time_period AND s.item_id = i.item_id
        ),
        
        -- 4. User-Level Aggregation (CTE Step 1)
        user_level_agg AS (
            SELECT 
                time_period,
                uid,
                segment,
                content_tier,
                1 AS has_activity,
                COUNT(DISTINCT CASE WHEN is_organic = 0 AND eligible THEN item_id END) AS user_algo_unique,
                COUNT(DISTINCT CASE WHEN is_organic = 1 AND eligible THEN item_id END) AS user_organic_unique,
                
                SUM(CASE WHEN eligible AND is_organic = 0 THEN 1 ELSE 0 END) AS algo_listens,
                SUM(CASE WHEN eligible AND is_organic = 1 THEN 1 ELSE 0 END) AS organic_listens,
                
                SUM(CASE WHEN eligible AND is_organic = 0 THEN played_seconds_capped ELSE 0 END) / 3600.0 AS algo_played_hours,
                SUM(CASE WHEN eligible AND is_organic = 1 THEN played_seconds_capped ELSE 0 END) / 3600.0 AS organic_played_hours,
                
                SUM(CASE WHEN eligible AND is_organic = 0 AND played_ratio_capped_pct >= 80 THEN 1 ELSE 0 END) AS algo_completed_listens,
                SUM(CASE WHEN eligible AND is_organic = 1 AND played_ratio_capped_pct >= 80 THEN 1 ELSE 0 END) AS organic_completed_listens,
                
                SUM(CASE WHEN eligible AND is_organic = 0 AND (played_seconds_capped <= 30 OR played_ratio_capped_pct < 10) THEN 1 ELSE 0 END) AS algo_short_listens,
                SUM(CASE WHEN eligible AND is_organic = 1 AND (played_seconds_capped <= 30 OR played_ratio_capped_pct < 10) THEN 1 ELSE 0 END) AS organic_short_listens,
                
                SUM(CASE WHEN event_type = 'like' AND is_organic = 0 THEN 1 ELSE 0 END) AS algo_likes,
                SUM(CASE WHEN event_type = 'like' AND is_organic = 1 THEN 1 ELSE 0 END) AS organic_likes,
                
                SUM(CASE WHEN event_type = 'unlike' AND is_organic = 0 THEN 1 ELSE 0 END) AS algo_unlikes,
                SUM(CASE WHEN event_type = 'unlike' AND is_organic = 1 THEN 1 ELSE 0 END) AS organic_unlikes,
                
                SUM(CASE WHEN event_type = 'dislike' AND is_organic = 0 THEN 1 ELSE 0 END) AS algo_dislikes,
                SUM(CASE WHEN event_type = 'dislike' AND is_organic = 1 THEN 1 ELSE 0 END) AS organic_dislikes
            FROM enriched_source
            GROUP BY time_period, uid, segment, content_tier
        )
        
        -- 5. Final Mart Aggregation (CTE Step 2)
        SELECT 
            time_period,
            segment,
            content_tier,
            SUM(has_activity) AS slice_users_count,
            COUNT(DISTINCT uid) AS segment_dau, -- DAU within the slice (Wait, Dau per segment is tricky here, but DataLens handles it)
            
            SUM(algo_listens) AS algo_listens,
            SUM(organic_listens) AS organic_listens,
            
            SUM(algo_played_hours) AS algo_played_hours,
            SUM(organic_played_hours) AS organic_played_hours,
            
            SUM(algo_completed_listens) AS algo_completed_listens,
            SUM(organic_completed_listens) AS organic_completed_listens,
            
            SUM(algo_short_listens) AS algo_short_listens,
            SUM(organic_short_listens) AS organic_short_listens,
            
            SUM(algo_likes) AS algo_likes,
            SUM(organic_likes) AS organic_likes,
            
            SUM(algo_unlikes) AS algo_unlikes,
            SUM(organic_unlikes) AS organic_unlikes,
            
            SUM(algo_dislikes) AS algo_dislikes,
            SUM(organic_dislikes) AS organic_dislikes,
            
            SUM(user_algo_unique) AS sum_algo_unique_items,
            SUM(user_organic_unique) AS sum_organic_unique_items
            
        FROM user_level_agg
        GROUP BY time_period, segment, content_tier
        ORDER BY time_period, segment, content_tier
        
    ) TO '{(marts_dir / 'mart_unified.parquet').as_posix()}' (FORMAT PARQUET, COMPRESSION 'ZSTD');
    """
    
    con.execute(query)
    logging.info("Mart compiled successfully.")
    shutil.rmtree(TMP_DIR, ignore_errors=True)

def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--input", type=str, default=str(DEFAULT_INPUT_GLOB))
    parser.add_argument("--marts-dir", type=Path, default=DEFAULT_MARTS_DIR)
    return parser.parse_args()

if __name__ == "__main__":
    args = parse_args()
    build_all(args.input, args.marts_dir.expanduser().resolve())
