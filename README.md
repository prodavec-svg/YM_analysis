...
├── test.ipynb                  # витринные графики для дашборда
├── requirements.txt
└── README.md
```
...
`test.ipynb` читает готовые витрины из `data/marts/` и строит витринные
графики для дашборда: динамику продукта по неделям, сегменты пользователей и
здоровье каталога по тирам.
